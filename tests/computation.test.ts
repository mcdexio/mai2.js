...
import { _tFunc, _rFunc, computeAccumulatedFunding } from '../src/computation'
import { DECIMALS, _0, _1 } from '../src/constants'
import { FundingGovParams, FundingParams } from '../src/types'
...
describe('funding', function() {
  ...
  getAccumulatedFundingCase1.forEach((element, index) => {
    ...
    it(`computeAccumulatedFunding.${index}:"${lastEMAPremium} ${lastPremium} ${timestamp}"`, function() {
      ...
      let err = expectedAcc.minus(acc).abs()
      expect(err.isLessThan(expectError)).toBeTruthy()
      err = expectedVT.minus(emaPremium).abs()
      expect(err.isLessThan(expectError)).toBeTruthy()
    })
  })
})
